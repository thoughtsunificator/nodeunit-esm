...
export const info = "Nested test reporter";
...
export const run = function (files, options, callback) {
		...
		let spaces_per_indent = options.spaces_per_indent || 4;
		...
		let pass_text = function (txt) {
				// Print in bold green.
				return bold(ok(txt + " (pass)"));
		};

		let fail_text = function (txt) {
				return bold(error(txt + " (fail) " + fail_indicator + " "));
		};

		let status_text = function (txt, status) {
				if (status === 'pass') {
		 ...
		 *   "TC1,TC1.1"
		 */
		let name_slice = function (name_arr, end_index) {
		...
		let add_indent = function (txt, indent_level) {
				let k;
		...
		let is_testCase = function (name_arr, index) {
				return index === name_arr.length - 1 ? false : true;
		};

		let testCase_line = function (txt) {
				return txt + "\n";
		};
		 ...
		 *         mytest (pass)
		 */
		let print_status = function (name_arr, status) {
				let txt = '';
				let _name_slice, part, i;
